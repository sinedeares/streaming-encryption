...
namespace streaming_encryption
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Начальное сообщение: " + initialText);
            CryptoService cryptoService = new CryptoService();
            byte[] data = Encoding.Unicode.GetBytes(initialText);
            Console.WriteLine("Введите ключ: ");
            string keyWord = Console.ReadLine();
            byte[] key = Encoding.Unicode.GetBytes(keyWord);


            byte[] cipherData = cryptoService.Encrypt(data, key);
            byte[] decryptedData = cryptoService.Decrypt(cipherData, key);

            string encryptedData = Encoding.Unicode.GetString(cipherData);
            Console.WriteLine("Зашифрованное сообщение " + encryptedData);

            string finalText = Encoding.Unicode.GetString(decryptedData);
